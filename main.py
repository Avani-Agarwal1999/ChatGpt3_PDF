...
def generate_summary(text, status_var):
    ...
    # Insert the summary into the output text widget
...
output_text = tk.Text(output_frame, width=80, wrap='word')
...
# Output text widget scrollbar
output_scrollbar = tk.Scrollbar(root, orient='vertical', command=output_text.yview)
output_scrollbar.pack(side='right', fill='y')
output_text.config(yscrollcommand=output_scrollbar.set)
